...
@triton.jit
def kernel_dot(Z):
    offsets_dmodel = tl.arange(0, 16)
    offsets_block = tl.arange(0, 16)
    offsets_q = 0 * 16 + 0 * 16 * 64 + offsets_dmodel * 64
    q = tl.load(Z + offsets_q)
    offsets_k = 0 * 16 + 0 * 16 * 64 + offsets_dmodel[None, :] * 64 + offsets_block[:, None] * 1
    k = tl.load(Z + offsets_k)
    S_ij = tl.zeros([16], dtype=tl.float32)
    S_ij += tl.sum(q[None, :] * k, 1)
    S_ij = S_ij.to(tl.float16)
    tl.store(Z + offsets_q, S_ij)
